...
public interface ComponentHost {
     ...
     * start its lifecycle, register it under {@code childId} for action routing, render it once), or
     * reuse the already-mounted instance on a later parent re-render (its {@code @State} survives —
     * the child is <em>not</em> re-created and, in 4b.2, not re-rendered here). Either way returns the
     * child's current VNode subtree rooted at {@code childId}.
     *
     ...
     */
    VNode mountChild(String childId, String name, Map<String, Object> params, int depth);
}

 ...
 *
 * <p>The sparkline island draws the points and handles hover entirely client-side (high-frequency,
 * zero server round-trips). Only a coarse "select" commit reaches the server, which records it in
 * {@code @State selected}; the re-render then pushes {@code data-selected} back to the island as a
 * host-attribute patch (props-down) and updates the label.</p>
 */
@MedleyRoute("/chart")

 ...
 * on the HTML the server actually serves. So the server emitted `{"op":"text","id":"root.1.2"}` — a TEXT
...
// Exactly what the server serves for /counter (span at root.1; `{{ label }}: {{ count }}`), pinned by
...
const COUNTER_SSR_BROKEN = '<span data-medley-id="root.1">Clicks: 0</span>';
...
test("a text patch updates what the user actually sees", () => withDom(COUNTER_SSR, (dom) => {
  ...
  medley.applyPatch({ op: "text", id: "root.1.2", value: "1" });
...
test("the patched host is an element, so the client can find it at all", () =>
  withDom(COUNTER_SSR, (dom) => {
    const host = dom.document.querySelector('[data-medley-id="root.1.2"]');
...
test("REGRESSION: bare dynamic text is unaddressable — the patch would be dropped", () =>
  withDom(COUNTER_SSR_BROKEN, (dom) => {
    ...
    assert.equal(dom.document.querySelector('[data-medley-id="root.1.2"]'), null);
